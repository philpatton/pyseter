...
abstract: Photographic identification (photo-ID) is an effective and non-invasive method for studying many aspects of animal ecology. Photo-ID, however, is a labor-intensive process that involves many steps including, but not limited to, grading individuals for distinctiveness (useful for partially marked populations), and curating the best image of each individual from each encounter. This latter step is helpful for reducing false negative errors, reducing the labor effort involved in manual or semi-automated identification, , and Photo-ID is a multi-step process that culminates in identifying individuals in a set of query images by comparing them against a reference set. Before this final identification step, there are several steps that should be taken to improve identification performance or downstream analyses. We introduce Pyseter, a Python package for automating several of these steps, including grading animals for distinctiveness, which is helpful for partially marked populations, and grouping query images into folders by a proposed ID then encounter, which is helpful for selecting the highest quality image of every individual by encounter. This curation can reduce false negative errors, and greatly reduces the effort involved in manual photo-ID. Pyseter includes AnyDorsal, an individual identification algorithm that was trained on 24 species of cetacean. As such, users can estimate similarity scores between individuals in cetacean dorsal images with Pyseter. We demonstrate a typical Pyseter workflow with a spinner dolphin example from Hawaiʻi. 
date: last-modified
